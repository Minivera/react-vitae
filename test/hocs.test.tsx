...
const hocs: {
  [s: string]: {
    hoc: (Component: never) => (props: never) => React.ReactElement<unknown>;
    expect: unknown;
  };
} = {
  withResume: {
...
Object.keys(hocs).forEach((hocName: string): void =>
  describe(`The ${hocName} hoc`, (): void => {
    ...
    const runContext = (): { [s: string]: unknown } | null => {
      ...
      // @ts-ignore
      const Tester = information.hoc((props: { [s: string]: unknown }): null => {
        contextValue = props;
        return null;
      });
